...
var Tabs = function () {
	function Tabs() {
		...
		this.el = {
			tabs: (0, _jquery2.default)('.tabs .tab'),
			panels: (0, _jquery2.default)('.panels .panel')
		};

		this.el.tabs.on('click', function (evt) {
			evt.preventDefault();_this.toggle((0, _jquery2.default)(evt.target));
		});
	}
...
var Articles = function (_Collection) {
	_inherits(Articles, _Collection);
	...
	_createClass(Articles, [{
		...
		value: function toggle(link) {
			var type = link.is(this.el.maps) ? 'map' : 'region';
			var fields = this.el.form.prop('elements');
			fields[type].value = link.data(type);

			var article = data.articles.find(function (article) {
				return article.map == fields.map.value && article.region == fields.region.value;
			});

			fields.id.value = article ? article.$loki : '';
			['title_sk', 'title_en', 'description_sk', 'description_en'].forEach(function (field) {
				return fields[field].value = article ? article[field.split('_')[0]][field.split('_')[1]] : '';
			});
...
var Pois = function (_Collection2) {
	_inherits(Pois, _Collection2);
	...
	_createClass(Pois, [{
		...
		value: function toggle(id) {
			...
			var poi = data.pois.find(function (poi) {
				return poi.$loki == id;
			});
		...
		value: function toggle(show) {
			...
			if (show && this.el.region.val() && this.el.map.val()) {
				...
				window.setTimeout(function () {
					var region = _this9.el.region.find(':selected').data();
					var coords = _this9.el.position.val().split(',');
					if (coords.length == 2) {
						var position = {
							left: _this9.el.picker.width() * (coords[0] - region.left) / region.width,
							top: _this9.el.picker.height() * (coords[1] - region.top) / region.height
						};
						(0, _jquery2.default)('<div class="pointer"></div>').css({ left: position.left + 'px', top: position.top + 'px' }).appendTo(_this9.el.picker);
					}
				}, 0.1 * 1000);
			} else this.el.picker.hide();
		}
	}, {
		key: 'point',
		value: function point(_ref) {
			var top = _ref.top,
			    left = _ref.left;

			var region = this.el.region.find(':selected').data();
			var position = {
